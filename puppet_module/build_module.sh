...
build() {
    ...
    docker run -t --rm -v ${outdir}/:/tmp/output:Z ${container_name} ${cyb3rhq_puppet_branch} || return 1
...
help() {
    ...
    echo -e ""
    echo -e "NAME"
    echo -e "        $(basename "${0}") - Build Cyb3rhq Puppet module."
    echo -e ""
    echo -e "SYNOPSIS"
    ...
    echo -e "                Enter the branch or tag of the cyb3rhq-puppet repository from which you want to build the module."
    echo -e ""
    echo -e "        -s, --store <path>"
...
main() {
    ...
    if [ -z "${cyb3rhq_puppet_branch}" ];  then
