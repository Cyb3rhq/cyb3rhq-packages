...
# Foundation.
Summary:     Cyb3rhq indexer is a search and analytics engine for security-related data. Documentation can be found at https://cyb3rhq.github.io/documentation/current/getting-started/components/cyb3rhq-indexer.html
...
%global INDEXER_FILE cyb3rhq-indexer-base-%{version}-%{release}-linux-x64.tar.xz

# -----------------------------------------------------------------------------

%description
Cyb3rhq indexer is a near real-time full-text search and analytics engine that gathers security-related data into one platform. This Cyb3rhq central component indexes and stores alerts generated by the Cyb3rhq server. Cyb3rhq indexer can be configured as a single-node or multi-node cluster, providing scalability and high availability. Documentation can be found at https://cyb3rhq.github.io/documentation/current/getting-started/components/cyb3rhq-indexer.html
...
chown -R %{USER}:%{GROUP} cyb3rhq-indexer-*/*

# Copy base files into RPM_BUILD_ROOT directory
mv cyb3rhq-indexer-*/etc ${RPM_BUILD_ROOT}/
mv cyb3rhq-indexer-*%{SYS_DIR}/* ${RPM_BUILD_ROOT}%{SYS_DIR}/
rm -rf cyb3rhq-indexer-*/etc
rm -rf cyb3rhq-indexer-*/usr
cp -pr cyb3rhq-indexer-*/* ${RPM_BUILD_ROOT}%{INSTALL_DIR}/

# Build cyb3rhq-certs-tool
%{REPO_DIR}/builder.sh -c

# Build cyb3rhq-passwords-tool
%{REPO_DIR}/builder.sh -p

# Copy the security tools
cp %{REPO_DIR}/cyb3rhq-certs-tool.sh ${RPM_BUILD_ROOT}%{INSTALL_DIR}/plugins/opensearch-security/tools/
cp %{REPO_DIR}/cyb3rhq-passwords-tool.sh ${RPM_BUILD_ROOT}%{INSTALL_DIR}/plugins/opensearch-security/tools/
cp /root/documentation-templates/cyb3rhq/config.yml ${RPM_BUILD_ROOT}%{INSTALL_DIR}/plugins/opensearch-security/tools/config.yml

# Copy Cyb3rhq's config files for the security plugin
...
chmod 750 ${RPM_BUILD_ROOT}/etc/init.d/cyb3rhq-indexer
...
if [ $1 = 2 ]; then
  ...
  # Check for SysV
    ...
    touch %{INSTALL_DIR}/%{name}.restart
      ...
      if command -v invoke-rc.d >/dev/null && invoke-rc.d --quiet cyb3rhq-indexer status > /dev/null 2>&1; then
...
if [ $1 = 1 ];then # Install
    ...
    # To pick up /usr/lib/sysctl.d/cyb3rhq-indexer.conf
...
if [ $1 = 0 ];then # Remove
    echo -n "Stopping cyb3rhq-indexer service..."
...
if [ -f %{INSTALL_DIR}/%{name}.restart ]; then
    echo -n "Starting cyb3rhq-indexer service..."
...
%attr(740, %{USER}, %{GROUP}) %{INSTALL_DIR}/plugins/opensearch-security/tools/cyb3rhq-certs-tool.sh
%attr(740, %{USER}, %{GROUP}) %{INSTALL_DIR}/plugins/opensearch-security/tools/cyb3rhq-passwords-tool.sh
