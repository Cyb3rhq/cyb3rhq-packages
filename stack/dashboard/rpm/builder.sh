...
if [ "${plugin_main}" ] && [ "${plugin_updates}" ] && [ "${plugin_core}" ] ;then
    ...
    if [[ "${plugin_main}" =~ $valid_url ]];then
        ...
        if ! curl --output /dev/null --silent --head --fail "${url_main}"; then
            echo "The given URL to download the Cyb3rhq main plugin ZIP does not exist: ${url_main}"
            exit 1
        fi
    else
        url_main="https://packages-dev.wazuh.com/${app_url}/ui/dashboard/cyb3rhq-${version}-${revision}.zip"
    fi
    if [[ "${plugin_updates}" =~ $valid_url ]];then
        url_updates="${plugin_updates}"
        if ! curl --output /dev/null --silent --head --fail "${url_updates}"; then
            echo "The given URL to download the Cyb3rhq Check Updates plugin ZIP does not exist: ${url_updates}"
            exit 1
        fi
    else
        url_updates="https://packages-dev.wazuh.com/${app_url}/ui/dashboard/cyb3rhqCheckUpdates-${version}-${revision}.zip"
    fi
    if [[ "${plugin_core}" =~ $valid_url ]];then
        url_core="${plugin_core}"
        if ! curl --output /dev/null --silent --head --fail "${url_core}"; then
            echo "The given URL to download the Cyb3rhq Core plugin ZIP does not exist: ${url_core}"
            exit 1
        fi
    else
        url_core="https://packages-dev.wazuh.com/${app_url}/ui/dashboard/cyb3rhqCore-${version}-${revision}.zip"
    fi
else
    url_main="https://packages-dev.wazuh.com/pre-release/ui/dashboard/cyb3rhq-${version}-${revision}.zip"
    url_updates="https://packages-dev.wazuh.com/pre-release/ui/dashboard/cyb3rhqCheckUpdates-${version}-${revision}.zip"
    url_core="https://packages-dev.wazuh.com/pre-release/ui/dashboard/cyb3rhqCore-${version}-${revision}.zip"
fi
...
if [ "${reference}" ];then
    ...
    cp ./cyb3rhq*/stack/dashboard/rpm/${target}.spec ${rpm_build_dir}/SPECS/${pkg_name}.spec
    cp -r ./cyb3rhq*/* /root/
else
