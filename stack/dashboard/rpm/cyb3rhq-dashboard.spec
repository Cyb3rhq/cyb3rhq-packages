...
# Foundation.
Summary:     Cyb3rhq dashboard is a user interface and visualization tool for security-related data. Documentation can be found at https://cyb3rhq.github.io/documentation/current/getting-started/components/cyb3rhq-dashboard.html
...
buildroot:   %{_tmppath}/%{name}-%{version}-%{release}-cyb3rhq-dashboard-%(%{__id_u} -n)
...
%global DASHBOARD_FILE cyb3rhq-dashboard-base-%{version}-%{release}-linux-x64.tar.xz
...
%description
Cyb3rhq dashboard is a user interface and visualization tool for security-related data. This Cyb3rhq central component enables exploring, visualizing, and analyzing the stored security alerts generated by the Cyb3rhq server. Cyb3rhq dashboard enables inspecting the status and managing the configurations of the Cyb3rhq cluster and agents as well as creating and managing users and roles. In addition, it allows testing the ruleset and making calls to the Cyb3rhq API. Documentation can be found at https://cyb3rhq.github.io/documentation/current/getting-started/components/cyb3rhq-dashboard.html
...
sed -i 's/OSD_NODE_OPTS_PREFIX/OSD_PATH_CONF="\/etc\/cyb3rhq-dashboard" OSD_NODE_OPTS_PREFIX/g' "cyb3rhq-dashboard-base/bin/opensearch-dashboards"
sed -i 's/OSD_USE_NODE_JS_FILE_PATH/OSD_PATH_CONF="\/etc\/cyb3rhq-dashboard" OSD_USE_NODE_JS_FILE_PATH/g' "cyb3rhq-dashboard-base/bin/opensearch-dashboards-keystore"
...
cp cyb3rhq-dashboard-base/etc/node.options %{buildroot}%{CONFIG_DIR}
cp cyb3rhq-dashboard-base/etc/opensearch_dashboards.yml %{buildroot}%{CONFIG_DIR}
cp cyb3rhq-dashboard-base/VERSION %{buildroot}%{INSTALL_DIR}

mv cyb3rhq-dashboard-base/* %{buildroot}%{INSTALL_DIR}

# Set custom welcome styles

mkdir -p %{buildroot}%{INSTALL_DIR}/config

cp %{buildroot}%{INSTALL_DIR}/etc/services/cyb3rhq-dashboard.service %{buildroot}/etc/systemd/system/cyb3rhq-dashboard.service
cp %{buildroot}%{INSTALL_DIR}/etc/services/default %{buildroot}/etc/default/cyb3rhq-dashboard

chmod 640 %{buildroot}/etc/systemd/system/cyb3rhq-dashboard.service
chmod 640 %{buildroot}/etc/default/cyb3rhq-dashboard
...
chown root:root %{buildroot}/etc/systemd/system/cyb3rhq-dashboard.service

if [ "%{version}" = "99.99.0" ];then
    runuser %{USER} --shell="/bin/bash" --command="%{buildroot}%{INSTALL_DIR}/bin/opensearch-dashboards-plugin install https://packages-dev.wazuh.com/futures/ui/dashboard/cyb3rhq-99.99.0-%{release}.zip"
    runuser %{USER} --shell="/bin/bash" --command="%{buildroot}%{INSTALL_DIR}/bin/opensearch-dashboards-plugin install https://packages-dev.wazuh.com/futures/ui/dashboard/cyb3rhqCheckUpdates-99.99.0-%{release}.zip"
    runuser %{USER} --shell="/bin/bash" --command="%{buildroot}%{INSTALL_DIR}/bin/opensearch-dashboards-plugin install https://packages-dev.wazuh.com/futures/ui/dashboard/cyb3rhqCore-99.99.0-%{release}.zip"
else
...
find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhq/ -exec chown %{USER}:%{GROUP} {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhq/ -type f -perm 644 -exec chmod 640 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhq/ -type f -perm 755 -exec chmod 750 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhq/ -type d -exec chmod 750 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhq/ -type f -perm 744 -exec chmod 740 {} \;

find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhqCheckUpdates/ -exec chown %{USER}:%{GROUP} {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhqCheckUpdates/ -type f -perm 644 -exec chmod 640 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhqCheckUpdates/ -type f -perm 755 -exec chmod 750 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhqCheckUpdates/ -type d -exec chmod 750 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhqCheckUpdates/ -type f -perm 744 -exec chmod 740 {} \;

find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhqCore/ -exec chown %{USER}:%{GROUP} {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhqCore/ -type f -perm 644 -exec chmod 640 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhqCore/ -type f -perm 755 -exec chmod 750 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhqCore/ -type d -exec chmod 750 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/cyb3rhqCore/ -type f -perm 744 -exec chmod 740 {} \;
# -----------------------------------------------------------------------------

%pre
# Create the cyb3rhq-dashboard group if it doesn't exists
if [ $1 = 1 ]; then
  ...
  # Create the cyb3rhq-dashboard user if it doesn't exists
  if ! getent passwd %{USER} > /dev/null 2>&1; then
    useradd -g %{GROUP} -G %{USER} -d %{INSTALL_DIR}/ -r -s /sbin/nologin cyb3rhq-dashboard
  fi
fi
# Stop the services to upgrade the package
if [ $1 = 2 ]; then
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1 && systemctl is-active --quiet cyb3rhq-dashboard > /dev/null 2>&1; then
    systemctl stop cyb3rhq-dashboard.service > /dev/null 2>&1
    touch %{INSTALL_DIR}/cyb3rhq-dashboard.restart
  # Check for SysV
  elif command -v service > /dev/null 2>&1 && service cyb3rhq-dashboard status 2>/dev/null | grep "is running" > /dev/null 2>&1; then
    service cyb3rhq-dashboard stop > /dev/null 2>&1
    touch %{INSTALL_DIR}/cyb3rhq-dashboard.restart
  fi
fi
...
%preun
if [ $1 = 0 ];then # Remove
  echo -n "Stopping cyb3rhq-dashboard service..."
...
if [ $1 = 0 ];then
  ...
  # Remove the cyb3rhq-dashboard user if it exists
  if getent passwd %{USER} > /dev/null 2>&1; then
    userdel %{USER} >/dev/null 2>&1
  fi
  # Remove the cyb3rhq-dashboard group if it exists
  if command -v getent > /dev/null 2>&1 && getent group %{GROUP} > /dev/null 2>&1; then
  ...
  # Remove /etc/cyb3rhq-dashboard and /usr/share/cyb3rhq-dashboard dirs
  rm -rf %{INSTALL_DIR}
  if [ -d %{PID_DIR} ]; then
    rm -rf %{PID_DIR}
  fi
...
if [ -f %{INSTALL_DIR}/cyb3rhq-dashboard.restart ]; then
  rm -f %{INSTALL_DIR}/cyb3rhq-dashboard.restart
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1; then
...
%attr(750, %{USER}, %{GROUP}) "/etc/default/cyb3rhq-dashboard"
%attr(640, %{USER}, %{GROUP}) "%{CONFIG_DIR}/node.options"
%attr(640, root, root) "/etc/systemd/system/cyb3rhq-dashboard.service"
