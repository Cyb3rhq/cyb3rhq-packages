...
build_deb() {
    ...
    if [ "${build_base}" == "yes" ];then
        ...
        basefile="${outdir}/cyb3rhq-dashboard-base-${version}-${revision}-linux-x64.tar.xz"
        if ! test -f "${basefile}"; then
            echo "Did not find expected Cyb3rhq dashboard base file: ${basefile} in output path. Exiting..."
            exit 1
        fi
    fi
...
help() {
    echo -e ""
    echo -e "NAME"
    echo -e "        $(basename "$0") - Build Cyb3rhq dashboard base file."
    echo -e ""
    echo -e "SYNOPSIS"
    ...
    echo -e "                [Optional] Cyb3rhq main plugin URL."
    echo -e ""
    echo -e "        -u, --updates-app <URL>"
    echo -e "                [Optional] Cyb3rhq Check Updates plugin URL."
    echo -e ""
    echo -e "        -c, --core-app <URL>"
    echo -e "                [Optional] Cyb3rhq Core plugin URL."
    echo -e ""
    ...
    echo -e "                [Optional] cyb3rhq-packages branch to download SPECs, not used by default."
    echo -e ""
...
main() {
    ...
    if [ ! "${plugin_main_reference}" ] && [ ! "${plugin_updates_reference}" ] && [ ! "${plugin_core_reference}" ]; then
        echo "No Cyb3rhq plugins have been defined, ${version} pre-release development packages with revision ${revision} will be used."
