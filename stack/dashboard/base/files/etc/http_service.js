  ...
  async runNotReadyServer(config) {
    ...
    this.notReadyServer.route({
      ...
      handler: (req, responseToolkit) => {
        this.log.debug(`Cyb3rhq dashboard server is not ready yet ${req.method}:${req.url.href}.`); // If server is not ready yet, because plugins or core can perform
        ...
        return responseToolkit.response('Cyb3rhq dashboard server is not ready yet').code(503).header('Retry-After', '30');
      }
    });
