...
if [ ${build_target} = "api" ]; then
    if [ "${local_source_code}" = "no" ]; then
        curl -sL https://github.com/cyb3rhq/cyb3rhq-api/tarball/${cyb3rhq_branch} | tar zx
    fi
    cyb3rhq_version="$(grep version cyb3rhq*/package.json | cut -d '"' -f 4)"
else
    if [ "${local_source_code}" = "no" ]; then
...
package_full_name="cyb3rhq-${build_target}-${cyb3rhq_version}"
sources_dir="${build_dir}/${build_target}/${package_full_name}"

mkdir -p ${build_dir}/${build_target}
cp -R cyb3rhq* ${build_dir}/${build_target}/cyb3rhq-${build_target}-${cyb3rhq_version}

if [ "${use_local_specs}" = "no" ]; then
    curl -sL https://github.com/cyb3rhq/cyb3rhq-packages/tarball/${cyb3rhq_packages_branch} | tar zx
    package_files="cyb3rhq*/debs"
...
if [[ "${future}" == "yes" ]]; then
    # MODIFY VARIABLES
    base_version=$cyb3rhq_version
    MAJOR=$(echo $base_version | cut -dv -f2 | cut -d. -f1)
    MINOR=$(echo $base_version | cut -d. -f2)
    cyb3rhq_version="${MAJOR}.30.0"
    file_name="cyb3rhq-${build_target}-${cyb3rhq_version}-${package_release}"
    old_name="cyb3rhq-${build_target}-${base_version}-${package_release}"
    package_full_name=cyb3rhq-${build_target}-${cyb3rhq_version}
    old_package_name=cyb3rhq-${build_target}-${base_version}
    mv "${build_dir}/${build_target}/${old_package_name}" "${build_dir}/${build_target}/${package_full_name}"
    sources_dir="${build_dir}/${build_target}/${package_full_name}"

    # PREPARE FUTURE SPECS AND SOURCES
    find "${build_dir}/${package_name}" "${specs_path}" \( -name "*VERSION*" -o -name "*changelog*" \) -exec sed -i "s/${base_version}/${cyb3rhq_version}/g" {} \;
    sed -i "s/\$(VERSION)/${MAJOR}.${MINOR}/g" "${build_dir}/${build_target}/${package_full_name}/src/Makefile"
    sed -i "s/${base_version}/${cyb3rhq_version}/g" "${build_dir}/${build_target}/${package_full_name}/src/init/cyb3rhq-server.sh"
    sed -i "s/${base_version}/${cyb3rhq_version}/g" "${build_dir}/${build_target}/${package_full_name}/src/init/cyb3rhq-client.sh"
    sed -i "s/${base_version}/${cyb3rhq_version}/g" "${build_dir}/${build_target}/${package_full_name}/src/init/cyb3rhq-local.sh"
fi
cp -pr ${specs_path}/cyb3rhq-${build_target}/debian ${sources_dir}/debian
...
if [ "${build_target}" == "api" ]; then
    sed -i "s:DIR=\"/var/ossec\":DIR=\"${dir_path}\":g" ${sources_dir}/debian/cyb3rhq-api.init
...
deb_file="cyb3rhq-${build_target}_${cyb3rhq_version}-${package_release}"
if [[ "${architecture_target}" == "ppc64le" ]]; then
...
mv ${pkg_path}/${deb_file} /var/local/cyb3rhq
