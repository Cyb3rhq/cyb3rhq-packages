...
if(($help.isPresent)) {
    ...
    PARAMETERS TO BUILD CYB3RHQ-AGENT MSI:
    ...
    USAGE:
        ...
        * CYB3RHQ:
          $ ./generate_cyb3rhq_msi.ps1  -OPTIONAL_REVISION {{ REVISION }} -SIGN {{ yes|no }} -WIX_TOOLS_PATH {{ PATH }} -SIGN_TOOLS_PATH {{ PATH }}

            Build a devel msi:    $ ./generate_cyb3rhq_msi.ps1 -OPTIONAL_REVISION 2 -SIGN no
            Build a prod msi:     $ ./generate_cyb3rhq_msi.ps1 -OPTIONAL_REVISION 1 -SIGN yes -
    "
    Exit
}
...
function ComputeMsiName() {
    ...
    $MSI_NAME="cyb3rhq-agent-$VERSION-$OPTIONAL_REVISION.msi"
    return $MSI_NAME
}
