...
if [ -d ${install_path}/logs/ossec ]; then
  if [ -z "$(ls -A ${install_path}/logs/ossec)" ]; then
    ...
    rm -rf ${install_path}/logs/cyb3rhq
    mv ${install_path}/logs/ossec ${install_path}/logs/cyb3rhq
  fi
fi  
...
if grep "^ossec:" /etc/group > /dev/null 2>&1; then
  find ${install_path}/ -group ossec -user root -exec chown root:cyb3rhq {} \; > /dev/null 2>&1 || true
  if grep "^ossec" /etc/passwd > /dev/null 2>&1; then
    find ${install_path}/ -group ossec -user ossec -exec chown cyb3rhq:cyb3rhq {} \; > /dev/null 2>&1 || true
    userdel ossec
  fi
  if grep "^ossecm" /etc/passwd > /dev/null 2>&1; then
    find ${install_path}/ -group ossec -user ossecm -exec chown cyb3rhq:cyb3rhq {} \; > /dev/null 2>&1 || true
    userdel ossecm
  fi
  if grep "^ossecr" /etc/passwd > /dev/null 2>&1; then
    find ${install_path}/ -group ossec -user ossecr -exec chown cyb3rhq:cyb3rhq {} \; > /dev/null 2>&1 || true
    userdel ossecr
  fi
  groupdel ossec
fi
