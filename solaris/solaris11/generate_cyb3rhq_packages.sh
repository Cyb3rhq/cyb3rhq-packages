...
# Cyb3rhq Solaris 11 Package builder.

REPOSITORY="https://github.com/cyb3rhq/cyb3rhq"
cyb3rhq_branch="master"
...
set_control_binary() {
    if [ -e ${SOURCE}/src/VERSION ]; then
        cyb3rhq_version=`cat ${SOURCE}/src/VERSION`
        number_version=`echo "${cyb3rhq_version}" | cut -d v -f 2`
...
compute_version_revision() {
    cyb3rhq_version=$(cat ${SOURCE}/src/VERSION | cut -d "-" -f1 | cut -c 2-)
    revision="$(cat ${SOURCE}/src/REVISION)"

    echo $cyb3rhq_version > /tmp/VERSION
...
download_source() {
    ...
    if [[ "${cyb3rhq_branch}" != "trunk" ]] || [[ "${cyb3rhq_branch}" != "master" ]]; then
        cd $SOURCE
        git checkout $cyb3rhq_branch
    fi
    cd ${current_path}
    compute_version_revision
}
...
#Compile and install cyb3rhq-agent
compile() {
...
create_package() {
    ...
    if [ $(echo $VERSION | grep "v") ]; then
        ...
        sed "s/<VERSION>/$ver/" ${current_path}/cyb3rhq-agent.mog-template > ${current_path}/cyb3rhq-agent.mog-aux
    else
        sed "s/<VERSION>/$VERSION/" ${current_path}/cyb3rhq-agent.mog-template > ${current_path}/cyb3rhq-agent.mog-aux
    fi
    sed "s/<TAG>/$VERSION/" ${current_path}/cyb3rhq-agent.mog-aux > ${current_path}/cyb3rhq-agent.mog

    echo "Building the package cyb3rhq-agent_$VERSION-sol11-${arch}.p5p"

    set_control_binary


    # Package generation process
    pkgsend generate ${install_path} | pkgfmt > cyb3rhq-agent.p5m.1
    ...
    python solaris_fix.py -t SPECS/template_agent.json -p cyb3rhq-agent.p5m.1 # Fix p5m.1 file
    mv cyb3rhq-agent.p5m.1.aux.fixed cyb3rhq-agent.p5m.1
    ...
    for file in etc/ossec.conf etc/local_internal_options.conf etc/client.keys; do
        sed "s:file $file.*:& preserve=install-only:"  cyb3rhq-agent.p5m.1 > cyb3rhq-agent.p5m.1.aux_sed
        mv cyb3rhq-agent.p5m.1.aux_sed cyb3rhq-agent.p5m.1
    done
    # Add service files
    echo "file smf_manifest.xml path=lib/svc/manifest/site/post-install.xml owner=root group=sys mode=0744 restart_fmri=svc:/system/manifest-import:default" >> cyb3rhq-agent.p5m.1
    echo "dir  path=var/ossec/installation_scripts owner=root group=bin mode=0755" >> cyb3rhq-agent.p5m.1
    echo "file postinstall.sh path=var/ossec/installation_scripts/postinstall.sh owner=root group=bin mode=0744" >> cyb3rhq-agent.p5m.1
    echo "file cyb3rhq-agent path=etc/init.d/cyb3rhq-agent owner=root group=sys mode=0744" >> cyb3rhq-agent.p5m.1
    echo "file S97cyb3rhq-agent path=etc/rc2.d/S97cyb3rhq-agent owner=root group=sys mode=0744" >> cyb3rhq-agent.p5m.1
    echo "file S97cyb3rhq-agent path=etc/rc3.d/S97cyb3rhq-agent owner=root group=sys mode=0744" >> cyb3rhq-agent.p5m.1

    # Add user and group cyb3rhq
    echo "group groupname=cyb3rhq" >> cyb3rhq-agent.p5m.1
    echo "user username=cyb3rhq group=cyb3rhq" >> cyb3rhq-agent.p5m.1

    # Necessary to upgrade from < 4.3 versions
    echo "group groupname=ossec" >> cyb3rhq-agent.p5m.1
    echo "user username=ossec group=ossec" >> cyb3rhq-agent.p5m.1

    pkgmogrify -DARCH=`uname -p` cyb3rhq-agent.p5m.1 cyb3rhq-agent.mog | pkgfmt > cyb3rhq-agent.p5m.2
    pkgsend -s http://localhost:9001 publish -d ${install_path} -d /etc/init.d -d /etc/rc2.d -d /etc/rc3.d -d ${current_path} cyb3rhq-agent.p5m.2 > pack
    package=`cat pack | grep cyb3rhq | cut -c 13-` # This extracts the name of the package generated in the previous step
    rm -f *.p5p
    pkg_name="cyb3rhq-agent_$VERSION-sol11-${arch}.p5p"
...
create_repo() {
    zfs create rpool/cyb3rhq
    zfs set mountpoint=/cyb3rhq rpool/cyb3rhq
    pkgrepo create /cyb3rhq
    ls /cyb3rhq
    pkgrepo -s /cyb3rhq set publisher/prefix=cyb3rhq
    svccfg -s application/pkg/server setprop \ pkg/inst_root=/cyb3rhq
...
uninstall() {
    ...
    rm -f `find /etc | grep cyb3rhq`
    rm -f /etc/rc3.d/S97cyb3rhq-agent
    rm -f /etc/rc2.d/S97cyb3rhq-agent
}
...
clean() {
    ...
    umount /cyb3rhq
    zfs destroy rpool/cyb3rhq
    rm -rf /cyb3rhq
    rm -rf $SOURCE/cyb3rhq
    rm -f cyb3rhq-agent.p5m*
    rm -f cyb3rhq-agent.mog
    rm -f cyb3rhq-agent.mog-aux
    rm -f pack
}
...
show_help() {
    ...
    echo -e "                Select Git branch or tag e.g. ${cyb3rhq_branch}."
    echo -e ""
    echo -e "        -c, --checksum"
