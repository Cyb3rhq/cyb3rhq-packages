...
function indexer_configure() {
    ...
    if [ -n "${AIO}" ]; then
        ...
        eval "installCommon_getConfig indexer/indexer_unattended_distributed.yml /etc/cyb3rhq-indexer/opensearch.yml ${debug}"
...
function indexer_startCluster() {
    ...
    eval "common_curl --silent ${filebeat_cyb3rhq_template} --max-time 300 --retry 5 --retry-delay 5 ${debug}" | eval "common_curl -X PUT 'https://${indexer_node_ips[pos]}:9200/_template/cyb3rhq' -H 'Content-Type: application/json' -d @- -uadmin:admin -k --silent --max-time 300 --retry 5 --retry-delay 5 ${debug}"
    if [  "${PIPESTATUS[0]}" != 0  ]; then
