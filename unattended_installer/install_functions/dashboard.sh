...
function dashboard_configure() {
    ...
    if [ -n "${AIO}" ]; then
        eval "installCommon_getConfig dashboard/dashboard_unattended.yml /etc/cyb3rhq-dashboard/opensearch_dashboards.yml ${debug}"
        dashboard_copyCertificates "${debug}"
    else
        eval "installCommon_getConfig dashboard/dashboard_unattended_distributed.yml /etc/cyb3rhq-dashboard/opensearch_dashboards.yml ${debug}"
        dashboard_copyCertificates "${debug}"
