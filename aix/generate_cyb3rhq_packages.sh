...
build_package() {
  ...
  source_code="http://api.github.com/repos/cyb3rhq/cyb3rhq/tarball/${reference}"

  rm -f cyb3rhq.tar.gz && curl -L ${source_code} -k -o cyb3rhq.tar.gz -s
  rm -rf cyb3rhq-cyb3rhq-* cyb3rhq-agent-*
  extracted_directory=$(gunzip -c cyb3rhq.tar.gz | tar -xvf - | tail -n 1 | cut -d' ' -f2 | cut -d'/' -f1)
  cyb3rhq_version=$(cat ${extracted_directory}/src/VERSION | cut -d'v' -f2)
  cp -pr ${extracted_directory} cyb3rhq-agent-${cyb3rhq_version}
  ...
  package_name=cyb3rhq-agent-${cyb3rhq_version}
  tar cf ${package_name}.tar ${package_name} && gzip ${package_name}.tar
  mv ${package_name}.tar.gz ${rpm_build_dir}/SOURCES/

  cp ${current_path}/SPECS/cyb3rhq-agent-aix.spec ${rpm_build_dir}/SPECS/${package_name}-aix.spec
