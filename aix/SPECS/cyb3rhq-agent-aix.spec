...
Summary:     The Cyb3rhq agent, used for threat detection, incident response and integrity monitoring.
...
%description
Cyb3rhq is an open source security monitoring solution for threat detection, integrity monitoring, incident response and compliance.
...
sed "s:CYB3RHQ_HOME_TMP:%{_localstatedir}:g" src/init/templates/ossec-hids-aix.init > src/init/templates/ossec-hids-aix.init.tmp
mv src/init/templates/ossec-hids-aix.init.tmp src/init/templates/ossec-hids-aix.init
/opt/freeware/bin/install -m 0750 src/init/templates/ossec-hids-aix.init ${RPM_BUILD_ROOT}%{_init_scripts}/cyb3rhq-agent
...
# Create cyb3rhq user and group
if ! grep "^cyb3rhq:" /etc/group > /dev/null 2>&1; then
  /usr/bin/mkgroup cyb3rhq
fi
if ! grep "^cyb3rhq" /etc/passwd > /dev/null 2>&1; then
  /usr/sbin/useradd cyb3rhq
  /usr/sbin/usermod -G cyb3rhq cyb3rhq
fi
...
if [ $1 = 2 ]; then
  if /etc/rc.d/init.d/cyb3rhq-agent status 2>/dev/null | grep "is running" > /dev/null 2>&1; then
    /etc/rc.d/init.d/cyb3rhq-agent stop > /dev/null 2>&1 || :
...
if [ $1 = 2 ]; then
  if [ -d %{_localstatedir}/logs/ossec ]; then
    cp -rp %{_localstatedir}/logs/ossec %{_localstatedir}/tmp/logs/cyb3rhq > /dev/null 2>&1
...
if [ $1 = 2 ]; then
  if [ -d %{_localstatedir}/tmp/logs/cyb3rhq ]; then
    rm -rf %{_localstatedir}/logs/cyb3rhq
    mv %{_localstatedir}/tmp/logs/ossec %{_localstatedir}/logs/cyb3rhq> /dev/null 2>&1
  fi
...
if [ $1 = 1 ]; then
  ...
  # Restore Cyb3rhq agent configuration
...
ln -fs /etc/rc.d/init.d/cyb3rhq-agent /etc/rc.d/rc2.d/S97cyb3rhq-agent
ln -fs /etc/rc.d/init.d/cyb3rhq-agent /etc/rc.d/rc3.d/S97cyb3rhq-agent
...
if [ -f %{_localstatedir}/tmp/cyb3rhq.restart ]; then
  ...
  /etc/rc.d/init.d/cyb3rhq-agent restart > /dev/null 2>&1 || :
fi
...
%dir %attr(750, cyb3rhq, cyb3rhq) %{_localstatedir}/ruleset/sca
%attr(640, root, cyb3rhq) %{_localstatedir}/ruleset/sca/*
%dir %attr(1750, root, cyb3rhq) %{_localstatedir}/tmp
...
%dir %attr(1750, root, cyb3rhq) %config(missingok) %{_localstatedir}/tmp/etc/templates
%dir %attr(1750, root, cyb3rhq) %config(missingok) %{_localstatedir}/tmp/etc/templates/config
%dir %attr(1750, root, cyb3rhq) %config(missingok) %{_localstatedir}/tmp/etc/templates/config/generic
%attr(750, root,system) %config(missingok) %{_localstatedir}/tmp/etc/templates/config/generic/*.template
%dir %attr(1750, root, cyb3rhq) %config(missingok) /var/ossec/tmp/etc/templates/config/generic/localfile-logs
