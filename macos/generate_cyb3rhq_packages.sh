...
function clean_and_exit() {
    ...
    rm "${CURRENT_PATH}"/specs/cyb3rhq-agent.pkgproj-e
...
function build_package() {
    ...
    ${CURRENT_PATH}/package_files/build.sh "${INSTALLATION_PATH}" "${CYB3RHQ_PATH}" ${JOBS}
    ...
    if packagesbuild ${AGENT_PKG_FILE} --build-folder ${DESTINATION} ; then
        ...
        pkg_name="cyb3rhq-agent-${VERSION}-${REVISION}.${ARCH}.pkg"
        sign_pkg
...
function get_pkgproj_specs() {
    ...
    VERSION=$(< "${CYB3RHQ_PATH}/src/VERSION"  cut -d "-" -f1 | cut -c 2-)

    pkg_file="specs/cyb3rhq-agent-${ARCH}.pkgproj"
...
function main() {
    ...
    if [[ "${BUILD}" != "no" ]]; then
        check_root
        AGENT_PKG_FILE="${CURRENT_PATH}/package_files/cyb3rhq-agent-${ARCH}.pkgproj"
        build_package
        "${CURRENT_PATH}/uninstall.sh"
    fi
    if [ "${NOTARIZE}" = "yes" ]; then
