...
# Needed variables to build Cyb3rhq with custom GCC and cmake
...
build_environment() {
    ...
    # Resizing partitions for Site Ox boxes (used by Cyb3rhq team)
...
compute_version_revision() {
    cyb3rhq_version=$(cat ${source_directory}/src/VERSION | cut -d "-" -f1 | cut -c 2-)

    echo ${cyb3rhq_version} > /tmp/VERSION
    echo ${cyb3rhq_revision} > /tmp/REVISION
...
download_source() {
    ...
    /usr/local/bin/curl -k -L -O https://github.com/cyb3rhq/cyb3rhq/archive/${cyb3rhq_branch}.zip
    /usr/local/bin/unzip ${cyb3rhq_branch}.zip
    mv cyb3rhq-* ${source_directory}
    compute_version_revision
}

check_version() {
    cyb3rhq_version=`cat ${source_directory}/src/VERSION`
...
create_package() {
    ...
    cyb3rhq_version=`echo "${cyb3rhq_version}" | cut -d v -f 2`
    pkg_tar_file="cyb3rhq-agent-${cyb3rhq_version}-${cyb3rhq_revision}-hpux-11v3-ia64.tar"
    tar cvpf ${target_dir}/${pkg_tar_file} ${install_path} /sbin/init.d/cyb3rhq-agent /sbin/rc2.d/S97cyb3rhq-agent /sbin/rc3.d/S97cyb3rhq-agent
...
clean() {
    ...
    find /sbin -name "*cyb3rhq-agent*" -exec rm {} \;
    userdel cyb3rhq
    groupdel cyb3rhq
