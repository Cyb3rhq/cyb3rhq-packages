...
if [[ "${future}" == "yes" ]]; then
    ...
    find "${sources_dir}" "${specs_path}" \( -name "*VERSION*" -o -name "*changelog*" \) -exec sed -i "s/${base_version}/${cyb3rhq_version}/g" {} \;
    sed -i "s/\$(VERSION)/${MAJOR}.${MINOR}/g" "${sources_dir}/src/Makefile"
    sed -i "s/${base_version}/${cyb3rhq_version}/g" "${sources_dir}/src/init/cyb3rhq-server.sh"
    sed -i "s/${base_version}/${cyb3rhq_version}/g" "${sources_dir}/src/init/cyb3rhq-client.sh"
    sed -i "s/${base_version}/${cyb3rhq_version}/g" "${sources_dir}/src/init/cyb3rhq-local.sh"
fi

cd ${sources_dir} && tar -czf ${pacman_dir}/${package_full_name}.tar.gz . 
cp -pr ${specs_path}/cyb3rhq-${build_target}/arch/* ${pacman_dir}

# Configure the package with the different parameters
sed -i "s:PARAM_VERSION:${cyb3rhq_version}:g" ${pacman_dir}/PKGBUILD
...
sed -i "s:PARAM_INSTALLATION_DIR:${dir_path}:g" ${pacman_dir}/cyb3rhq.install
sed -i "s:PARAM_INSTALLATION_SCRIPTS_DIR:${dir_path}/packages_files/agent_installation_scripts:g" ${pacman_dir}/cyb3rhq.install
...
pkg_file="cyb3rhq-${build_target}-${cyb3rhq_version}-${package_release}-${architecture_target}.pkg.tar.zst"
pkg_path="${pacman_dir}"

if [[ "${checksum}" == "yes" ]]; then
    cd ${pkg_path} && sha512sum ${pkg_file} > /var/local/checksum/${pkg_file}.sha512
fi
install -o root -g root ${pkg_path}/${pkg_file} /var/local/cyb3rhq/${pkg_file}
