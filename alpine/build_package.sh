...
build_apk() {
    ...
    volumes="-v ${outdir}/:/var/local/cyb3rhq:Z"
    ...
    if [ "${local_source}" != "no" ];then
        volumes="${volumes} -v ${local_source}:/cyb3rhq:Z"
    fi
...
help() {
    ...
    echo -e "               [Required] Select Git branch or tag from cyb3rhq repository."
    echo -e ""
    ...
    echo -e ""
    echo -e "       --sources <path>"
    echo -e "               [Optional] Absolute path containing cyb3rhq source code. This option will use local source code instead of downloading it from GitHub."
    echo -e ""
    echo -e "       --packages-reference <ref>"
    echo -e "               [Optional] Select Git branch or tag from cyb3rhq-packages repository. e.g ${spec_reference}"
    echo -e ""
    echo -e "       --dev"
