...
packages_list=( "main_cyb3rhq" "cyb3rhq-check-updates_cyb3rhqCheckUpdates" "cyb3rhq-core_cyb3rhqCore" )
...
cyb3rhq_app_nvmrc_url="${cyb3rhq_app_raw_repo_url}/${cyb3rhq_branch}/.nvmrc"
...
prepare_env() {
    echo "Downloading package.json and .nvmrc from cyb3rhq-dashboard-plugins repository"
    if ! curl $cyb3rhq_app_package_json_url -o "/tmp/package.json" ; then
        echo "Error downloading package.json from GitHub."
        exit 1
    fi

    if ! curl $cyb3rhq_app_nvmrc_url -o "/tmp/.nvmrc" ; then
...
download_cyb3rhq_app_sources() {
    if ! git clone $cyb3rhq_app_clone_repo_url --branch ${cyb3rhq_branch} --depth=1 ${git_clone_tmp_dir}; then
        echo "Error downloading the source code from cyb3rhq-dashboard-app GitHub repository."
        exit 1
    fi
...
build_package(){
    ...
    for item in ${packages_list[@]}; do
        ...
        if [ -z "${app_revision}" ]; then
            cyb3rhq_app_pkg_name="${array[1]}-${cyb3rhq_version}.zip"
        else
            cyb3rhq_app_pkg_name="${array[1]}-${cyb3rhq_version}-${app_revision}.zip"
        fi
        ...
        find "${source_dir}/${array[0]}" -name "*.zip" -exec mv {} ${destination_dir}/${cyb3rhq_app_pkg_name} \;
