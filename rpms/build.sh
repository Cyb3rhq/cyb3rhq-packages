...
cp -R cyb3rhq-* ${build_dir}/${package_name}
...
if [ "${use_local_specs}" = "no" ]; then
    ...
    curl -L "https://raw.githubusercontent.com/cyb3rhq/cyb3rhq-packages/${cyb3rhq_packages_branch}/rpms/SPECS/cyb3rhq-${build_target}.spec" -o "${specs_path}/cyb3rhq-${build_target}.spec" ${ssl_flag}
else
    specs_path="/specs"
fi
...
if [[ "${future}" == "yes" ]]; then
    ...
    find "${build_dir}/${package_name}" "${specs_path}/" \( -name "*VERSION*" -o -name "*.spec" \) -exec sed -i "s/${base_version}/${cyb3rhq_version}/g" {} \;
    sed -i "s/\$(VERSION)/${MAJOR}.${MINOR}/g" "${build_dir}/${package_name}/src/Makefile"
    sed -i "s/${base_version}/${cyb3rhq_version}/g" "${build_dir}/${package_name}/src/init/cyb3rhq-server.sh"
    sed -i "s/${base_version}/${cyb3rhq_version}/g" "${build_dir}/${package_name}/src/init/cyb3rhq-client.sh"
    sed -i "s/${base_version}/${cyb3rhq_version}/g" "${build_dir}/${package_name}/src/init/cyb3rhq-local.sh"
fi
...
find ${extract_path} -maxdepth 3 -type f -name "${file_name}*" -exec mv {} /var/local/cyb3rhq \;
