...
clean() {
    ...
    # Clean the files
    rm -rf ${DOCKERFILE_PATH}/{*.tar.gz,cyb3rhq*} ${DOCKERFILE_PATH}/build.sh ${SOURCES_DIRECTORY}
...
build_rpm() {
    ...
    if [ ! -z "${LOCAL_SOURCE_CODE}" ]; then
        CUSTOM_CODE_VOL="-v ${LOCAL_SOURCE_CODE}:/cyb3rhq-local-src:Z"
...
help() {
    ...
    echo "    --sources <path>             [Optional] Absolute path containing cyb3rhq source code. This option will use local source code instead of downloading it from GitHub."
    echo "    --packages-branch <branch>   [Optional] Select Git branch or tag from cyb3rhq-packages repository. e.g ${PACKAGES_BRANCH}"
